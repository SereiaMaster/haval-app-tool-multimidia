...
const focusableAreas = {
    ...
    accent_selection: ['accent_title', 'accent_blue', 'accent_yellow', 'accent_red', 'accent_orange'],
};
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'u' || e.key === 'U') {
        ...
        const i = Math.max(0, order.indexOf(currentState.uiAccent || 'blue'));
        window.control('uiAccent', order[(i + 1) % order.length]);
        return;
    }
    ...
    if (currentScreen === 'main_menu') {
        ...
        if (e.key === 'ArrowUp') {
            ...
            if (currentState.focusedMenuItem === 'option_1') {
                ...
                const a = stateManager.get('uiAccent') || 'blue';
                setState('accentFocus', `accent_${a}`);
                window.showScreen('accent_selection');
    ...
    } else if (currentScreen === 'accent_selection') {
        const controls = focusableAreas.accent_selection;
        const currentFocus = currentState.accentFocus || 'accent_blue';
        const currentIndex = Math.max(0, controls.indexOf(currentFocus));

        if (e.key === 'ArrowUp') {
            let prevIndex = (currentIndex - 1 + controls.length) % controls.length;
            if (controls[prevIndex] === 'accent_title') {
                prevIndex = (prevIndex - 1 + controls.length) % controls.length;
            }
            window.focus(controls[prevIndex]);
        } else if (e.key === 'ArrowDown') {
            let nextIndex = (currentIndex + 1) % controls.length;
            if (controls[nextIndex] === 'accent_title') {
                nextIndex = (nextIndex + 1) % controls.length;
            }
            window.focus(controls[nextIndex]);
        } else if (e.key === 'Enter') {
            if (currentFocus.startsWith('accent_') && currentFocus !== 'accent_title') {
                const color = currentFocus.replace('accent_', '');
                window.control('uiAccent', color);
            }
        }
    }
