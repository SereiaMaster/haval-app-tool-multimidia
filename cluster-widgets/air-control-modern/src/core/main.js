...
import { createAccentSelectionScreen } from './components/display/accentSelection.js';
...
const UI_ACCENTS = ['blue', 'yellow', 'red', 'orange'];

function syncUiAccentClass() {
    const raw = get('uiAccent');
    const key = UI_ACCENTS.includes(raw) ? raw : 'blue';
    document.documentElement.classList.remove('accent-blue', 'accent-yellow', 'accent-red', 'accent-orange');
    document.documentElement.classList.add(`accent-${key}`);
}

subscribe('uiAccent', syncUiAccentClass);
...
function initializeLayout() {
    ...
    try {
        ...
        appContainer.classList.add('cockpit-modern-root');
...
function render() {
    ...
    if (screenCache[screen]) {
        ...
        try {
            ...
            } else if (screen === 'accent_selection') {
                componentResult = createAccentSelectionScreen();
